...
// Handler registry
const handlerRegistry: Record<string, (input: Record<string, unknown>) => Promise<any>> = {
  build: buildHandler,
...
const formatOutput = (data: any, title?: string) => {
  if (title) {
  ...
  async lint(args) {
    ...
    if (data.valid) {
      ...
      if (data.warnings?.length) {
        ...
        data.warnings.forEach((w: any) => console.log(`  - ${w}`));
      }
    } else {
      console.error(`${colors.red}✗${colors.reset} Invalid spec`);
      data.errors?.forEach((e: any) => console.error(`  - ${e}`));
      process.exit(1);
    }
