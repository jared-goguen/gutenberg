...
import type { FooterData } from "./footer.data.js";
import { createNode } from "../scaffold/node.js";

export function scaffoldFooter(data: FooterData): RenderNode {
  const children: RenderNode[] = [];
  
  // Top row: logo/description + links + social (if variant is detailed)
  if (data.variant === "detailed") {
    const topRowChildren: RenderNode[] = [];
    
    // Left column: logo + description
    if (data.logo || data.description) {
      const logoDescColumn: (RenderNode | string)[] = [];
    ...
    if (data.social && data.social.length > 0) {
      topRowChildren.push(
        createNode("div", {
          ...
          children: data.social.map(social =>
            createNode("a", {
