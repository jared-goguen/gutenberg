...
export async function handler(input: Record<string, unknown>) {
  ...
  try {
    ...
    let lintContent: string;
    let enrichContent: string;

    try {
      lintContent = await fs.readFile(lint_path, "utf8");
    } catch (error) {
      throw new Error(
        `LINT artifact not found at ${lint_path}.\n` +
    ...
    try {
      ...
      throw new Error(
        `ENRICH artifact not found at ${enrich_path}.\n` +
    ...
    let annotatedNodes: AnnotatedNode[];

    try {
      const lintData = JSON.parse(lintContent);
      schema = lintData.schema as PageSchema;
      if (!schema) {
        throw new Error("LINT artifact missing 'schema' field");
      }
    } catch (error) {
      throw new Error(
        `Failed to parse LINT artifact at ${lint_path}: ${error instanceof Error ? error.message : String(error)}`
      );
    }

    try {
      const enrichData = JSON.parse(enrichContent);
      annotatedNodes = enrichData.nodes as AnnotatedNode[];
      if (!Array.isArray(annotatedNodes)) {
        throw new Error("ENRICH artifact 'nodes' field is not an array");
      }
    } catch (error) {
      throw new Error(
        `Failed to parse ENRICH artifact at ${enrich_path}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
