...
async function readDirectory(dirPath: string): Promise<Record<string, string>> {
...
async function cfAssetFetch(path: string, jwt: string, options: RequestInit = {}): Promise<unknown> {
...
export async function handler(input: Record<string, unknown>) {
  ...
  const { jwt } = await cfFetch<{ jwt: string }>(
    `/accounts/${accountId}/pages/projects/${project_name}/upload-token`
  );
