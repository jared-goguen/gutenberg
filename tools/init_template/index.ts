...
export async function handler(input: Record<string, unknown>) {
  ...
  const templateName = input.name as string;
 ...
 */
function generateTemplate(name: string, route: string, param: string): string {
 ...
 */
function generateWorkerFunction(templateName: string, param: string): string {
  return `/**
 ...
 */
async function ensureWranglerConfig(wranglerPath: string): Promise<void> {
  try {
