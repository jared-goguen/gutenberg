...
function getTableCellColorClass(attrs) {
    ...
    if (cellType === "numeric" && normalizedStr !== undefined) {
        ...
        if (!invertColors) {
            ...
            if (normalized < 0.2)
                colorClass = "cell-heat-very-low";
            else if (normalized < 0.4)
                colorClass = "cell-heat-low";
            else if (normalized < 0.6)
                colorClass = "cell-heat-mid";
            else if (normalized < 0.8)
                colorClass = "cell-heat-high";
            else
                colorClass = "cell-heat-very-high";
        }
        else {
 ...
 */
export function enrichRenderNodes(nodes) {
    return nodes.map(enrichNode);
}
/**
 * Enrich a single RenderNode
 */
export function enrichRenderNode(node) {
