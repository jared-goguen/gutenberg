 ...
 */
async function handleRender(config, paramValue) {
    try {
        ...
        if (issues.length > 0) {
            ...
            if (errors.length > 0) {
                return new Response(`Validation error: ${JSON.stringify(errors, null, 2)}`, { status: 400, headers: { 'Content-Type': 'application/json' } });
            }
        }
        sanitizeSpec(spec);
 ...
 */
async function handleEditRender(config, paramValue) {
    try {
        const bucket = config.bucket;
        ...
        const templateRaw = YAML.parse(templateYaml);
 ...
 */
async function handleSave(config, paramValue, request) {
    try {
        const bucket = config.bucket;
        ...
        const errors = issues.filter(i => i.severity === 'error');
        if (errors.length > 0) {
            return new Response(`Validation error on save: ${JSON.stringify(errors, null, 2)}`, { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
...
function formDataToYAML(formData, template, paramValue, routeParam) {
    ...
    if (spec.title) {
        spec.title = spec.title.replace(new RegExp(paramPlaceholder, 'g'), paramValue);
    }
 ...
 */
function replacePlaceholders(block, placeholder, value) {
