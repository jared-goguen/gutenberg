 ...
 */
export function serializeHTML(node, options = {}) {
...
function serializeNode(node, depth, options) {
    ...
    if (!node || typeof node !== "object") {
        throw new Error(`Invalid node: expected AnnotatedNode or string, got ${typeof node}`);
    }
    if (!("tag" in node) || typeof node.tag !== "string") {
        throw new Error(`Invalid AnnotatedNode: missing or invalid 'tag' property (got ${typeof node.tag})`);
    }
    ...
    if (attrs !== null && typeof attrs !== "object") {
        throw new Error(`Invalid node property 'attrs' on <${tag}>: expected object or null, got ${typeof attrs}`);
    }
    if (!Array.isArray(classes)) {
        throw new Error(`Invalid node property 'classes' on <${tag}>: expected array, got ${typeof classes}`);
    }
    if (!Array.isArray(children)) {
        throw new Error(`Invalid node property 'children' on <${tag}>: expected array, got ${typeof children}`);
    }
    // Void element
    ...
    if (rawHtml !== undefined) {
        if (typeof rawHtml !== "string") {
            throw new Error(`Invalid node property 'rawHtml' on <${tag}>: expected string, got ${typeof rawHtml}`);
        }
 ...
 */
export function serializeHTMLNodes(nodes, options = {}) {
    return nodes
