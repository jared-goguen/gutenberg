...
function lintAll(source, enabled) {
    ...
    const schema = validateSpec(spec).map((i) => ({
        severity: i.severity,
        check: `schema:${i.path}`,
        message: i.message,
    }));
    ...
    const visual = visualLint(spec, enabled).map((i) => ({
        severity: i.severity,
        check: i.check,
        message: i.message,
        block: i.block,
...
function formatIssues(issues) {
    ...
    const body = issues
        .map((i) => `[${i.severity.toUpperCase().padEnd(7)}] ${i.check}${i.block !== undefined ? ` (block ${i.block})` : ""}: ${i.message}`)
        .join("\n");
    return body ? `${header}\n${body}` : header;
}
...
function findProjectRoot(dir) {
    let current = dir;
    for (let i = 0; i < 20; i++) {
        if (existsSync(join(current, "_site.yaml")) || existsSync(join(current, "_project.yaml")))
            return current;
    ...
    if (directory) {
        try {
            ...
            if (!stat.isDirectory()) {
                return {
                    content: [
                        { type: "text", text: `Error: not a directory: ${directory}` },
                    ],
                    isError: true,
                };
            }
        ...
        if (files.length === 0) {
            return {
                content: [
                    { type: "text", text: `No .yaml files found in ${directory}` },
                ],
            };
        }
        const results = [];
        ...
        const conventionByFile = new Map();
        ...
        const cleanFiles = results.filter((r) => r.issues.length === 0 && !r.error).length;
    ...
    if (filePath) {
        try {
            ...
            if (!stat.isFile()) {
                return {
                    content: [
                        { type: "text", text: `Error: not a file: ${filePath}` },
                    ],
                    isError: true,
                };
            }
